...
interface NovedadesListClientProps {
  novedades: any[];
  filterProduct?: string;
  uniqueProducts: string[];
}

export default function NovedadesListClient({
  novedades,
  filterProduct,
  uniqueProducts,
}: NovedadesListClientProps) {
  const { t, language } = useTranslation();
  const recentPosts = novedades.slice(0, 5);

  const filteredNovedades = filterProduct
    ? novedades.filter((n: any) => n.product?.title === filterProduct)
    : novedades;
    ...
    <div className="max-w-[1400px] mx-auto py-12 px-4 md:px-8">
      ...
      <div className="mb-10">
        <h1 className="text-4xl md:text-5xl font-poppins font-bold text-neutral-900">
          {t('news.title', 'Novedades')}
        </h1>
        <p className="text-lg text-neutral-800 font-poppins max-w-2xl mt-2">
          {t('news.subtitle', 'Mantente al día con los últimos lanzamientos de productos, actualizaciones técnicas, integraciones y guías de Seidor Analytics.')}
        </p>
      </div>
      ...
      <div className="flex sm:flex-col lg:flex-row gap-10 items-start">
        ...
        <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredNovedades.length === 0 ? (
            <div className="col-span-full text-center py-20 bg-white dark:bg-neutral-800 rounded-3xl border border-neutral-200 dark:border-neutral-700">
              <h2 className="text-xl font-poppins font-semibold text-neutral-800 dark:text-neutral-200 mb-2">
                {t('news.noNews', 'No hay noticias')}
              </h2>
              <p className="text-neutral-500 font-poppins">
                {t('news.noNewsSubtitle', 'Vuelve pronto para enterarte de lo nuevo.')}
              </p>
            </div>
          ) : (
              ...
              >
                ...
                <div className="flex flex-col p-6 flex-1">
                  <div className="flex items-center justify-between gap-3 mb-4">
                    ...
                    <time className="text-[12px] text-neutral-400 dark:text-neutral-100 font-medium font-poppins">
                      {new Date(novedad.created_at).toLocaleDateString(language === 'en' ? 'en-US' : 'es-ES', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </time>
                  </div>
        ...
        <aside className="sm:w-full lg:w-[380px] shrink-0 bg-white dark:bg-neutral-800 p-8 rounded-3xl border border-neutral-200 dark:border-neutral-700 sticky top-[80px]">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 className="text-xl font-bold font-poppins text-neutral-900 dark:text-white">
              {t('news.recentPosts', 'Posteos Recientes')}
            </h3>
          </div>
            ...
            {recentPosts.map((novedad: any, index: number) => (
              <li key={novedad.id} className={`group ${index !== recentPosts.length - 1 ? 'border-b border-neutral-100 dark:border-neutral-700 pb-6' : ''}`}>
                <Link href={`/novedades/${novedad.id}`} className="flex flex-col gap-2">
                  ...
                  <span className="text-sm font-semibold text-neutral-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-snug font-poppins">
