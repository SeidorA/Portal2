...
export default function DocumentosPage() {
  ...
  const handleCreate = async () => {
    ...
    if (error) {
      console.error(error);
      alert(t('documents.errorCreating', 'Error creando el archivo'));
    } else if (data) {
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-end justify-between bg-container rounded-lg p-4">
        <div>
          <h1 className="text-2xl font-bold font-poppins text-neutral-900">
            {t('documents.title', 'Documentos y Presentaciones')}
          </h1>
          <p className="text-sm text-neutral-800 mt-1">
            {t('documents.subtitle', 'Gestiona tus documentos de texto y presentaciones interactivas.')}
          </p>
          ...
          <div className="w-full mt-4">
            <Tabs
              tabs={[
                { label: t('documents.tabDocuments', 'Documentos') },
                { label: t('documents.tabPresentations', 'Presentaciones') }
              ]}
        ...
        <Button variant="info" onClick={() => setIsModalOpen(true)} iconName='plus' size='md'>
          {activeTab === 'documentos' ? t('documents.createDocument', 'Crear Documento') : t('documents.createPresentation', 'Crear Presentación')}
        </Button>
      </div>
      ...
      {loading ? (
        <div className="flex justify-center p-12">
          <span className="text-neutral-500 animate-pulse">{t('common.loading', 'Cargando...')}</span>
        </div>
        ...
        <div className="flex flex-col items-center justify-center p-12 bg-container rounded-lg border border-neutral-200 dark:border-neutral-800 shadow-sm mt-4">
          ...
          <h3 className="text-lg text-neutral-900">
            {activeTab === 'documentos' ? t('documents.emptyStateTitleDoc', 'Aún no hay documentos') : t('documents.emptyStateTitlePres', 'Aún no hay presentaciones')}
          </h3>
          <p className="text-sm text-neutral-800 mt-2 text-center max-w-sm">
            {activeTab === 'documentos' ? t('documents.emptyStateDescDoc', 'Haz clic en el botón superior derecho para crear tu primer documento.') : t('documents.emptyStateDescPres', 'Haz clic en el botón superior derecho para crear tu primera presentación.')}
          </p>
        </div>
      ) : (
                ...
                <div className="flex justify-between items-start gap-2">
                  ...
                  >
                    <div className="flex align-end gap-1 mb-2">
                      <span className='mb-0 text-neutral-800 font-semibold'>{t('documents.createdBy', 'Creado por:')}</span>
                      <p className='mb-0 text-neutral-900 font-semibold truncate max-w-[120px]' title={doc.edit_history?.[0]?.user || t('documents.unknownUser', 'Desconocido')}>
                        {doc.edit_history?.[0]?.user ? doc.edit_history[0].user.split('@')[0] : t('documents.unknownUser', 'Desconocido')}
                      </p>
                    </div>
                    <p className="text-xs text-neutral-800">
                      {t('documents.lastEdited', 'Última edición:')} {new Date(doc.updated_at).toLocaleDateString()}
                    </p>
                  </div>
                  ...
                  {doc.content?.metadata?.status === 'published' ? (
                    <span className="text-[10px] font-bold px-2 py-1 bg-success-main/90 text-white rounded-full uppercase tracking-wider backdrop-blur-sm">
                      {t('documents.statusPublished', 'Publicado')}
                    </span>
                  ) : (
                ...
                <div className="flex justify-between gap-1 mt-4 text-sm">
                  ...
                  <div className="w-[30%] text-center">
                    <span className={doc.content?.metadata?.restriction === 'internal' ? 'text-danger-main' : 'text-neutral-800'}>
                      {doc.content?.metadata?.restriction === 'internal' ? t('documents.internalUse', 'Uso interno') :
                        doc.content?.metadata?.restriction === 'restricted' ? t('documents.restricted', 'Restringido') : t('documents.public', 'Público')}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-container rounded-xl w-full max-w-md p-6 shadow-xl flex flex-col gap-4 animate-scale-in">
            <h2 className="text-xl font-bold text-neutral-900 font-poppins">
              {activeTab === 'documentos' ? t('documents.newDocument', 'Nuevo Documento') : t('documents.newPresentation', 'Nueva Presentación')}
            </h2>
            <p className="text-sm text-neutral-500">
              {t('documents.enterTitleToStart', 'Ingresa un título para comenzar.')}
            </p>

            <TextInput
              label={t('documents.titleField', 'Título')}
              placeholder={t('documents.titlePlaceholder', 'Ej. Brochure Comercial')}
              value={newTitle}
            ...
            <div className="flex justify-end gap-2 mt-2">
              <Button variant="ghost" onClick={() => { setIsModalOpen(false); setNewTitle(''); }} disabled={isCreating}>
                {t('documents.cancel', 'Cancelar')}
              </Button>
              <Button variant="info" onClick={handleCreate} disabled={!newTitle.trim() || isCreating}>
                {isCreating ? t('documents.creating', 'Creando...') : t('documents.create', 'Crear')}
              </Button>
            </div>
          </div>
        </div>
      ...
      <Drawer
        ...
        title={selectedDoc?.title || t('documents.tabDocuments', 'Documento')}
      >
        {selectedDoc && (
          <div className="flex flex-col gap-8">
            ...
            <div className="bg-neutral-500 border border-neutral-200 dark:border-neutral-800 rounded-xl p-4 relative">
              ...
              <h3 className="text-base font-bold text-neutral-900 mb-1">{t('documents.shareDoc', 'Compartir documento')}</h3>
              <p className="text-xs text-neutral-800 mb-4 max-w-[85%]">
                {t('documents.shareNotice', 'Recuerda que los documentos pueden tener restricciones de acceso.')}
              </p>
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  {copyStatus === 'link' ? t('documents.copied', '¡Copiado!') : t('documents.copyLink', 'Copiar link')}
                </Button>
                <Button variant="info" className="w-full text-sm"
                  iconName={copyStatus === 'insert' ? 'check' : 'link'}
                  onClick={handleCopyInsert}
                >
                  {copyStatus === 'insert' ? t('documents.copied', '¡Copiado!') : t('documents.insertCode', 'Insertar')}
                </Button>
              </div>
            </div>
            ...
            {/* Historial */}
            <div>
              <h3 className="text-base font-bold text-neutral-900 mb-4">{t('documents.history', 'Historial')}</h3>
                    ...
                    >
                      <div className="flex flex-col mb-4 -mt-2 ml-2 bg-full rounded-lg p-3">
                        ...
                        <p className="text-xs font-semibold text-neutral-900">
                          {hist.action || t('documents.statusDraft', 'Editado')} {t('documents.createdBy', 'por:')} <br /><span className="font-normal text-neutral-600">{hist.user}</span>
                        </p>
                      </div>
                    </Timeline>
                  ))}
                  {selectedDoc.edit_history.length > 3 && (
                    <div className="flex justify-center mt-2 relative z-10">
                      <Button variant="ghost" size="sm"
                        iconName='clock'
                      >
                        {t('documents.viewFullHistory', 'Ver todo el historial')}
                      </Button>
                    </div>
                  )}
                </div>
              ) : (
                <p className="text-sm text-neutral-500">{t('documents.noHistory', 'No hay historial disponible.')}</p>
              )}
            </div>

            {/* Informacion */}
            <div>
              <h3 className="text-base font-bold text-neutral-900 mb-4">{t('documents.infoTitle', 'Información')}</h3>
              <div className="flex flex-col gap-3">
                <div className="flex gap-2">
                  <span className="text-sm text-neutral-800 w-1/3">{t('documents.createdBy', 'Creado Por')}</span>
                  <span className="text-sm font-semibold text-neutral-900 flex-1 truncate" title={selectedDoc.edit_history?.[0]?.user || t('documents.unknownUser', 'Desconocido')}>
                    {selectedDoc.edit_history?.[0]?.user || t('documents.unknownUser', 'Desconocido')}
                  </span>
                </div>
                <div className="flex gap-2">
                  <span className="text-sm text-neutral-800 w-1/3">{t('documents.docLanguage', 'Idioma del Documento')}</span>
                  <span className="text-sm font-semibold text-neutral-900 flex-1">
                    {selectedDoc.content?.metadata?.language === 'en' ? t('documents.langEn', 'Inglés') :
                      selectedDoc.content?.metadata?.language === 'pt' ? t('documents.langPt', 'Portugués') :
                        selectedDoc.content?.metadata?.language === 'de' ? t('documents.langDe', 'Alemán') : t('documents.langEs', 'Español')}
                  </span>
                </div>
                <div className="flex gap-2">
                  <span className="text-sm text-neutral-800 w-1/3">{t('documents.restriction', 'Restricción')}</span>
                  <span className="text-sm font-semibold text-neutral-900 flex-1">
                    {selectedDoc.content?.metadata?.restriction === 'internal' ? t('documents.internalOnlyEmployees', 'Interno (solo empleados)') :
                      selectedDoc.content?.metadata?.restriction === 'restricted' ? t('documents.restricted', 'Restringido') : t('documents.public', 'Público')}
                  </span>
                </div>
                {selectedDoc.content?.metadata?.tags && (
                  <div className="flex gap-2 items-start mt-1">
                    <span className="text-sm text-neutral-800 w-1/3 mt-1">{t('documents.tags', 'Tags')}</span>
                ...
                <div className="flex gap-2 mt-1">
                  <span className="text-sm text-neutral-800 w-1/3">{t('documents.format', 'Formato')}</span>
                  <span className="text-sm font-semibold text-neutral-900 flex-1">
