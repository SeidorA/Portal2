...
export default function NovedadesPage() {
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    if (!title.trim() || !productId || !content.trim()) {
      alert(t("newsAdmin.validationRequired", "Título, producto y contenido son requeridos."));
      return;
    }
    ...
    try {
      ...
      alert(t("newsAdmin.saveError", "Error guardando novedad: ") + e.message);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(t("newsAdmin.deleteConfirm", "¿Estás seguro de eliminar esta novedad?"))) return;
    try {
      ...
      alert(t("newsAdmin.deleteError", "Error eliminando: ") + e.message);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto py-8 px-4 h-full">
      <div className="flex flex-row justify-between items-end">
        <div className="flex flex-col">
          <h1 className="text-3xl text-neutral-900 dark:text-white font-poppins font-bold">
            {t("newsAdmin.title", "Novedades (Blog)")}
          </h1>
          <p className="text-neutral-600 dark:text-neutral-400 mt-2 font-poppins">
            {t("newsAdmin.subtitle", "Gestiona las entradas públicas de novedades asociadas a los productos.")}
          </p>
        </div>
        <Button size="l" color="primary" onClick={() => openDrawer()}>
          <CaralIcon name="plus" size="s" />
          {t("newsAdmin.newPost", "Nueva Novedad")}
        </Button>
      </div>
      ...
      <div className="w-full flex-1 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-sm overflow-hidden mt-6 flex flex-col">
        {loading ? (
          <div className="p-8 text-center text-neutral-500 font-poppins">{t("newsAdmin.loading", "Cargando...")}</div>
        ) : novedades.length === 0 ? (
          <div className="p-8 text-center text-neutral-500 font-poppins">{t("newsAdmin.empty", "No hay novedades registradas.")}</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-neutral-600 dark:text-neutral-400">
              <thead className="bg-neutral-50 dark:bg-neutral-800/50 text-neutral-900 dark:text-white font-poppins border-b border-neutral-200 dark:border-neutral-800">
                <tr>
                  <th className="px-6 py-4 font-semibold">{t("newsAdmin.tableTitle", "Título")}</th>
                  <th className="px-6 py-4 font-semibold">{t("newsAdmin.tableProduct", "Producto")}</th>
                  <th className="px-6 py-4 font-semibold">{t("newsAdmin.tableStatus", "Estado")}</th>
                  <th className="px-6 py-4 font-semibold">{t("newsAdmin.tableDate", "Fecha")}</th>
                  <th className="px-6 py-4 font-semibold text-right">{t("newsAdmin.tableActions", "Acciones")}</th>
                </tr>
              </thead>
              <tbody>
                {novedades.map((n) => (
                  <tr key={n.id} className="border-b border-neutral-100 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800/30 transition-colors">
                    <td className="px-6 py-4 font-medium text-neutral-900 dark:text-white truncate max-w-xs font-poppins">
                      {n.title}
                    </td>
                    <td className="px-6 py-4 font-poppins">
                      {n.product?.title || t("newsAdmin.unknown", "Desconocido")}
                    </td>
                    <td className="px-6 py-4 font-poppins">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${n.status === 'published' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'}`}>
                        {n.status === 'published' ? t("newsAdmin.statusPublished", "Publicado") : t("newsAdmin.statusDraft", "Borrador")}
                      </span>
                    </td>
                    <td className="px-6 py-4 font-poppins">
                    ...
                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                      <Button variant="ghost" size="s" onClick={() => openDrawer(n)} title={t("newsAdmin.edit", "Editar")}>
                      ...
                      <Button
                        ...
                        title={t("newsAdmin.delete", "Eliminar")}
                      >
        ...
        <div className="flex flex-col h-full relative">
          ...
          <div className="flex-none p-6 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between sticky top-0 bg-white dark:bg-neutral-900 z-10">
            <h2 className="text-xl font-poppins font-bold text-neutral-900 dark:text-white">
              {currentId ? t("newsAdmin.editTitle", "Editar Novedad") : t("newsAdmin.createTitle", "Nueva Novedad")}
            </h2>
            <div className="flex items-center gap-2">
              {currentId && (
                ...
                >
                  <CaralIcon name="trash" size="s" />
                  {t("newsAdmin.delete", "Eliminar")}
                </Button>
              )}
              <Button type="button" variant="ghost" onClick={closeDrawer}>
                {t("newsAdmin.cancel", "Cancelar")}
              </Button>
              <Button type="button" color="primary" loading={isSaving} onClick={handleSave}>
                {t("newsAdmin.save", "Guardar")}
              </Button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              <label className="font-poppins font-medium text-sm text-neutral-700 dark:text-neutral-300">
                {t("newsAdmin.fieldTitle", "Título de la Novedad")}
              </label>
              <input
                type="text"
                value={title}
                ...
                required
                className="w-full bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg px-4 py-2 outline-none focus:border-blue-500 font-poppins text-neutral-900 dark:text-white"
                placeholder={t("newsAdmin.fieldTitlePlaceholder", "Ej. Nueva actualización v2.0")}
              />
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex flex-col gap-2 flex-1">
                <label className="font-poppins font-medium text-sm text-neutral-700 dark:text-neutral-300">
                  {t("newsAdmin.fieldProduct", "Producto Asociado")}
                </label>
                <select
                  value={productId}
                  ...
                  className="w-full bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg px-4 py-2 outline-none focus:border-blue-500 font-poppins text-neutral-900 dark:text-white"
                >
                  <option value="" disabled>{t("newsAdmin.fieldProductSelect", "Selecciona un producto")}</option>
                  {products.map(p => (
              ...
              <div className="flex flex-col gap-2 flex-1">
                <label className="font-poppins font-medium text-sm text-neutral-700 dark:text-neutral-300">
                  {t("newsAdmin.fieldStatus", "Estado")}
                </label>
                <select
                  value={status}
                  ...
                  className="w-full bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg px-4 py-2 outline-none focus:border-blue-500 font-poppins text-neutral-900 dark:text-white"
                >
                  <option value="published">{t("newsAdmin.statusPublished", "Publicado")}</option>
                  <option value="draft">{t("newsAdmin.statusDraft", "Borrador")}</option>
                </select>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4 p-5 bg-neutral-50 dark:bg-neutral-800/40 border border-neutral-200 dark:border-neutral-700/80 rounded-xl">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <label className="font-poppins font-semibold text-sm text-neutral-900 dark:text-white">
                    {t("newsAdmin.customization", "Personalización de Entrada (Fondo y Texto)")}
                  </label>
                </div>
                {(bgColor || bgImage || textColor) && (
                  <button
                    ...
                    className="text-xs text-blue-600 hover:text-blue-700 dark:text-blue-400 font-medium hover:underline cursor-pointer font-poppins"
                  >
                    {t("newsAdmin.resetDefaults", "Restablecer valores por defecto")}
                  </button>
                )}
              </div>
              <p className="text-xs text-neutral-500 dark:text-neutral-400 -mt-2 font-poppins">
                {t("newsAdmin.customizationHelp", "Opcional. Configura el fondo y color de texto tanto para la vista web como para el correo copiado. Si no se modifican, se mantienen los valores por defecto actuales.")}
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
                ...
                <div className="flex flex-col gap-2">
                  <label className="font-poppins font-medium text-xs text-neutral-700 dark:text-neutral-300">
                    {t("newsAdmin.bgColor", "Color de Fondo")}
                  </label>
                  <div className="flex items-center gap-2">
                    <input
                      type="color"
                      ...
                      title={t("newsAdmin.bgColor", "Color de Fondo")}
                    />
                    <input
                      type="text"
                      placeholder={t("newsAdmin.colorDefaultAuto", "Por defecto (Auto)")}
                      value={bgColor}
                      onChange={(e) => setBgColor(e.target.value)}
                      className="flex-1 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg px-3 py-2 text-xs font-mono outline-none focus:border-blue-500 text-neutral-900 dark:text-white"
                    />
                    {bgColor && (
                      <button
                        type="button"
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-[10px] text-neutral-400 font-poppins">Presets:</span>
                    {[
                ...
                <div className="flex flex-col gap-2">
                  <label className="font-poppins font-medium text-xs text-neutral-700 dark:text-neutral-300">
                    {t("newsAdmin.textColor", "Color de Texto")}
                  </label>
                  <div className="flex items-center gap-2">
                    <input
                      type="color"
                      ...
                      className="w-10 h-10 rounded-lg border border-neutral-300 dark:border-neutral-700 cursor-pointer p-0.5 bg-white dark:bg-neutral-800"
                      title={t("newsAdmin.textColor", "Color de Texto")}
                    />
                    <input
                      type="text"
                      placeholder={t("newsAdmin.colorDefaultAuto", "Por defecto (Auto)")}
              ...
              <div className="flex flex-col gap-2 pt-2 border-t border-neutral-200 dark:border-neutral-700/60">
                <label className="font-poppins font-medium text-xs text-neutral-700 dark:text-neutral-300">
                  {t("newsAdmin.bgImagePattern", "Imagen o Patrón de Fondo (Opcional)")}
                </label>
                <div className="flex flex-col gap-2">
                  <FileUploader
              ...
              {(bgColor || bgImage || textColor) && (
                <div className="mt-2 flex flex-col gap-1.5">
                  <span className="text-[11px] font-semibold text-neutral-500 uppercase tracking-wider font-poppins">
                    {t("newsAdmin.previewStyle", "Vista previa de estilo")}
                  </span>
                  <div
                    style={{
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2 opacity-80 text-xs font-poppins">
                      <span className="px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-500 font-medium">Producto</span>
                      <span>Fecha</span>
                    </div>
                    <h4 style={{ color: textColor || undefined }} className="font-bold text-base mb-1 font-poppins">
                      {title || t("newsAdmin.previewSampleTitle", "Título de ejemplo de la novedad")}
                    </h4>
                    <p style={{ color: textColor ? `${textColor}cc` : undefined }} className="text-xs opacity-90 font-poppins">
                      {t("newsAdmin.previewSampleDesc", "Así se visualizará el fondo y texto de tu novedad tanto en el portal como en el correo copiado.")}
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="flex flex-col gap-2">
              <label className="font-poppins font-medium text-sm text-neutral-700 dark:text-neutral-300">
                {t("newsAdmin.coverImage", "Imagen de Portada (Opcional)")}
              </label>
              <FileUploader
            ...
            <div className="flex flex-col gap-2 flex-1 min-h-[700px]">
              <label className="font-poppins font-medium text-sm text-neutral-700 dark:text-neutral-300">
                {t("newsAdmin.contentMarkdown", "Contenido (Markdown)")}
              </label>
            ...
            {currentId && (
              <div className="mt-8 pt-6 border-t border-neutral-200 dark:border-neutral-800">
                <h3 className="text-lg font-bold text-neutral-900 dark:text-white font-poppins">
                  {t("newsAdmin.dangerZone", "Zona de peligro")}
                </h3>
                <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-4 font-poppins">
                  {t("newsAdmin.dangerZoneHelp", "Esta acción no se puede deshacer.")}
                </p>
                ...
                <div className="border border-red-200 dark:border-red-900/50 bg-[#FDEEED] dark:bg-red-950/20 rounded-xl p-4 flex items-center justify-between gap-4">
                  <div>
                    <h4 className="font-bold text-[#641A1B] dark:text-red-400 mb-1 text-sm font-poppins">
                      {t("newsAdmin.deletePostTitle", "Eliminar esta Novedad")}
                    </h4>
                    <p className="text-xs text-[#641A1B] dark:text-red-500 font-poppins">
                      {t("newsAdmin.deletePostHelp", "La entrada de novedad se eliminará permanentemente de la base de datos.")}
                    </p>
                  </div>
                  <Button
                    type="button"
