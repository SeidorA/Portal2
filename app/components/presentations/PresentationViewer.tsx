...
export default function PresentationViewer({ content, title }: PresentationViewerProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 w-full min-h-0 flex items-center justify-center relative p-3 md:p-6 overflow-hidden">
        {/* 16:9 Presentation Frame */}
        <div
          className="aspect-video w-full max-h-full bg-white dark:bg-neutral-950 rounded-2xl shadow-2xl border border-neutral-800 flex flex-col relative overflow-hidden transition-all duration-200 shrink-0"
          style={{
            aspectRatio: '16 / 9',
            maxHeight: isFullscreen ? '96vh' : 'calc(100vh - 120px)',
            maxWidth: isFullscreen ? 'min(98vw, calc(96vh * 16 / 9))' : 'min(96vw, calc((100vh - 120px) * 16 / 9), 1440px)',
            width: '100%',
            height: 'auto',
          }}
        >
            ...
            >
              {currentSlide.columns.map((col, cIdx) => {
                ...
                return (
                  <div
                    key={col.id || cIdx}
                    className={`h-full flex flex-col relative ${isImageCol
                        ? 'p-0 overflow-hidden'
                  ...
                  >
                    {isImageCol ? (
                      ...
                      <div
                        className={`w-full flex flex-col ${isAutoHeight ? 'h-auto max-h-full' : `h-full ${colVAlignClass}`
                          } ${colPaddingClass} ${colGlassClass} overflow-hidden transition-all`}
                      >
                          ...
                          {col.blocks && col.blocks.length > 0 ? (
                            col.blocks.map((b) => (
                              <div key={b.id}>
                                {b.type === 'title' ? (
                                  <h2
                                    style={getBlockInlineStyle(b.style, 36)}
                                    className="text-3xl xl:text-4xl 2xl:text-5xl font-extrabold font-poppins text-neutral-900 dark:text-neutral-100 tracking-tight leading-tight mb-4"
                                  >
                                    {b.title || b.content}
                                  </h2>
                                ) : b.type === 'explicativo' ? (
                                  <div className={`flex items-start transition-all ${b.size === 'large' ? 'gap-4 my-3 xl:my-5' : b.size === 'small' ? 'gap-2 my-1' : 'gap-3 my-2'
                                    }`}>
                                    <div className={`flex-shrink-0 text-neutral-900 dark:text-neutral-100 ${b.size === 'large' ? 'mt-1' : 'mt-0.5'
                                      }`}>
                                      <CaralIcon
                                        name={(b.icon as any) || 'settings'}
                                        size={
                                          b.size === 'large' ? 32 : b.size === 'small' ? 18 : 24
                                        }
                                      />
                                    </div>
                                    <div className="flex-1 space-y-1">
                                      <h3
                                        style={getBlockInlineStyle(
                                          b.style,
                                          b.size === 'large' ? 22 : b.size === 'small' ? 14 : 18
                                        )}
                                        className={`font-extrabold font-poppins text-neutral-900 dark:text-neutral-100 tracking-tight leading-snug ${b.size === 'large'
                                            ? 'text-xl xl:text-2xl'
                                            : b.size === 'small'
                                              ? 'text-sm'
                                      ...
                                      {b.description && (
                                        <p
                                          className={`text-neutral-600 dark:text-neutral-400 font-normal leading-relaxed whitespace-pre-line ${b.size === 'large'
                                              ? 'text-md xl:text-base mt-1'
                                              : b.size === 'small'
                                                ? 'text-xs'
                                  ...
                                  <div
                                    className={`w-full flex ${b.imageAlign === 'left'
                                        ? 'justify-start'
                                        : b.imageAlign === 'right'
                                          ? 'justify-end'
                                          : 'justify-center'
                                    ...
                                    >
                                      {b.imageUrl ? (
                                        <img
                                          ...
                                          className={`w-full h-auto max-h-[420px] object-cover ${b.imagePadding && b.imagePadding !== '0'
