...
export default function DocumentCover({
  ...
  const defaultUntitled = t('documents.untitled', 'Sin Título');
  if (coverSettings.titleMode === 'custom') {
    renderTitle = <h1 className="text-5xl font-extrabold font-poppins text-neutral-900 dark:text-white tracking-tight leading-tight max-w-3xl drop-shadow-sm">{coverSettings.customTitle || title || defaultUntitled}</h1>;
  } else if (coverSettings.titleMode === 'name') {
    renderTitle = <h1 className="text-5xl font-extrabold font-poppins text-neutral-900 dark:text-white tracking-tight leading-tight max-w-3xl drop-shadow-sm">{firstProduct ? firstProduct.title : title || defaultUntitled}</h1>;
  } else {
    renderTitle = (
      <div className="flex items-center justify-center gap-6">
        ...
        <h1 className="text-[57px]! font-extrabold font-poppins text-[#07153A] leading-tight drop-shadow-sm">{firstProduct ? firstProduct.title : title || defaultUntitled}</h1>
      </div>
    );
  }
            ...
            if (p.icon_name) {
              return (
                <div key={p.id} className="text-blue-500 flex items-center gap-4 drop-shadow-md">
                  ...
                  <h1 className="text-[18px]! font-extrabold text-[#07153A] leading-tight drop-shadow-sm">{firstProduct ? firstProduct.title : title || defaultUntitled}</h1>
